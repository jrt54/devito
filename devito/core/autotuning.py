...
def generate_block_shapes(blockable, args, level):
    ...
    # A) Generate level-0 block shapes
    level0 = [d for d, v in mapper.items() if v == 0]

    # Max attemptable block shape
    max_bs = tuple((d.step.name, d.max_step.subs(args)) for d in level0)
    from IPython import embed; embed()

    # Attempted block shapes:
    # A1) Defaults (basic mode)
    ret = [tuple((d.step.name, v) for d in level0) for v in options['blocksize-l0']]
    # A2) Always try the entire iteration space (degenerate block)
    ret.append(max_bs)
    # A3) More attempts if auto-tuning in aggressive mode
    ...
    # A1) Block shapes exceeding the iteration space extent
    ret = [i for i in ret if all(dict(i)[k] <= v for k, v in max_bs)]
    # A2) Redundant block shapes
    ret = filter_ordered(ret)

    # B) Generate level-1 block shapes
    level1 = [d for d, v in mapper.items() if v == 1]
    # TODO

    # C) Generate level-n (n > 1) block shapes
    # Essentially a no-op
    ret = [i + ]
